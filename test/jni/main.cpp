...
int main() {
    ...
    int r_cse = cse_len_sum();

    std::printf("ALLVM_TEST_VALUES a=%d b=%d ref=%d cse=%d\n",
                a, b, r_ref, r_cse);
    std::fprintf(stderr, "ALLVM_TEST_VALUES a=%d b=%d ref=%d cse=%d\n",
                 a, b, r_ref, r_cse);

    int expect_len = static_cast<int>(std::strlen("CSE_TEST_STRING_hello") +
                                      std::strlen("中文_世界_字符串"));
    if (r_ref != (a + b) || r_cse != expect_len) {
